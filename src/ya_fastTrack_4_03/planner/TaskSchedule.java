...
public class TaskSchedule {
    static class Task {
        ...
        void assignWorker(Worker worker) {
            this.worker = worker;
            this.startExecuted = worker.getTimeWhenReady() < this.queuingTime
                    ? this.queuingTime
                    : worker.timeWhenReady;
        }
        @Override
        public String toString() {return "T{" + "id= " + id + ", s=" + queuingTime + '}';}
    }
    ...
    static class Worker {
        ...
        void updateTimeWhenReady(Task task) {
            this.timeWhenReady = task.startExecuted + task.duration;
        }
        int getId() {return id;}
    ...
    static class QueueManager {
        ...
        @Override
        public String toString() {
            return "{" + "QM=" + queueNumber + ", tasks=" + tasksQueue + '}';
        }
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        /*PriorityQueue<QueueManager> managerQueue = new PriorityQueue<>(Comparator.comparing(QueueManager::getUsage)
                .thenComparing(QueueManager::getPriority)
                .thenComparing(QueueManager::getQueueNumber));*/

        /*PriorityQueue<QueueManager> managerQueue = new PriorityQueue<>((a,b) -> {
            if (a.usage != b.usage) {
                return a.usage - b.usage;
            } else {
               long timeWhenReady = workersQueue.peek().getTimeWhenReady();
        ...
        /*while (!managerQueue.isEmpty()) {
            ...
            if (!qm.tasksQueue.isEmpty()) {
                ...
                managerQueue.offer(qm);

                //updatePriority(managerQueue, workersQueue.peek());
            } else {
                workersQueue.offer(worker);
            }
        }*/
        ...
        for (int i = 0; i < n; i++){
            ...
            //System.out.println(managerQueue);
